...
<SAURE_TODO>
表 3-1 原始 `JointState` 链路的隐含信任假设表占位。
建议包含字段：
- 信任假设；
- 正常情况下的作用；
- 被打破后的风险后果。
...
- 后续章节中的防护对应点。
建议说明的问题：
- 建立“威胁 -> 风险 -> 防护目标”之间的映射关系；
- 便于第六节防护设计回扣本节。
建议排版：
- 四列表格；
- 可将“后续章节中的防护对应点”简写为“XSB 规则层 / 代码层”。
</SAURE_TODO>
...
在攻击版规则模型中，规则层围绕攻击链的自然推进顺序构造，核心可分为四个阶段。

第一阶段是**发现与学习阶段**。规则 `enumerateTopic(...)` 表示攻击者在可达 ROS2 域且分布式发现开启时可以枚举系统可见 topic；规则 `learnMessageSchema(...)` 则表示攻击者在枚举到目标 topic 并知道其消息类型后，可以进一步学习消息格式。该阶段对应实验中通过 `ros2 topic list`、`ros2 topic type` 与 `ros2 interface show` 获得 `/joint_states` 结构信息的过程。

第二阶段是**合法伪造阶段**。规则 `forgeValidJointState(...)` 不直接假设攻击者“想发就能发”，而是要求攻击者同时满足关节名称匹配、关节范围已知、当前时间戳可生成等条件。正是由于这一规则显式纳入了时间戳与语义合法性约束，模型才能解释为什么最初直接用 `ros2 topic pub` 注入失败，而使用 Python publisher 补充有效时间戳后攻击成功。

第三阶段是**显示层攻击阶段**。规则 `injectForgedJointState(...)` 表示在缺少 topic 认证和 publisher ACL 的条件下，攻击者可以向关键状态流中注入伪造消息；规则 `tamperTFStream(...)` 则将这种伪造进一步映射到 `robot_state_publisher` 所生成的 TF 链；规则 `misleadPoseDisplay(...)` 最终表示 RViz 会据此得到被篡改的姿态显示。到此，攻击链 A 从“枚举 topic”逐步演化为“误导显示层状态”。

第四阶段是**扩展攻击与传播阶段**。规则 `competingPublisherState(...)` 用于表达合法 publisher 与恶意 publisher 在同一未受保护 topic 上并存的情形，规则 `oscillatingPoseDisplay(...)` 则把这一竞争状态映射为姿态振荡结果；同时，规则 `propagateToBridge(...)` 与 `physicalLayerRisk(...)` 又进一步刻画了：如果下游存在继续信任 `JointState` 的桥接节点，那么显示层攻击可以向控制层甚至物理层扩展。

通过这种分阶段组织方式，攻击版规则模型并不是单纯给出一个结论，而是构造了一条可解释的攻击逻辑链。它既能支撑顶层查询，也能借助 `spy(...)` 将每一步为何成立展示出来，从而与实验现象形成清晰映射。
...
在实现层面，攻击脚本基于 `rclpy` 编写，并使用 `sensor_msgs.msg.JointState` 作为消息类型。与直接命令行注入相比，其关键改动主要体现在三个方面。

第一，脚本在每次发布消息时都显式设置：

`msg.header.stamp = self.get_clock().now().to_msg()`

这一操作使得伪造消息具备当前有效时间戳，从而不再停留在“topic 中的静态数字变化”，而能够真正进入姿态更新链路。

第二，脚本中的 `msg.name` 严格使用当前机械臂模型中的关节名称顺序，包括 `joint2_to_joint1`、`joint3_to_joint2` 等六个关节名。这样可以保证下游节点在解释消息时不会因名称不一致而忽略输入。

第三，脚本中的 `msg.position` 严格限制在实验确认的关节范围内。为增强演示效果，本文分别使用了固定姿态注入脚本与连续异常摆动脚本：前者用于验证“攻击是否成立”，后者用于展示“攻击者如何持续控制显示层状态变化”。

从实现意义上看，这一实验说明攻击者真正需要掌握的并不是某条单独命令，而是目标系统对消息语义的接受条件。也正因为如此，攻击脚本与后续防护脚本在逻辑上具有天然镜像关系：前者尽量满足这些条件，后者则尽量检查并约束这些条件。
...
<SAURE_TODO>
表 5-2 `ros2 topic pub` 失败与 Python publisher 成功的对照表占位。
建议包含字段：
- 注入方式；
- 时间戳情况；
- `/joint_states` 是否变化；
...
<SAURE_TODO>
代码段 6-1 `trusted_joint_state_filter` 核心校验逻辑占位。
建议内容：
- 只截取最关键部分：
  - 原始 topic 订阅；
  - publisher 数量检查；
...
需要强调的是，本文实现的防护方案是一套**最小应用层防护原型**，它的设计目标是尽可能直接地对应结项阶段已经复现出的攻击链，而不是取代完整的 ROS2 安全体系。
...
但与此同时，这套防护也存在明显边界。首先，publisher 数量检查本质上是一种工程启发式策略，而不是真正的密码学身份认证；其次，如果攻击者在某种极端场景下成为唯一消息源，并且仍能构造时间戳、关节名和关节范围均合法的消息，则单纯的应用层检查并不能完全等价于正式的安全框架；最后，该方案尚未引入 SROS2、DDS Security、证书认证或更细粒度的 topic ACL，因此不能把它表述为“完整的 ROS2 安全防护体系”。

因此，本文更稳妥的结论是：当前方案已经能够对本文所复现的主攻击链形成有效阻断，并验证了“输入分层 + 条件校验 + 权威源约束”这一防护思路的工程可行性；而更强、更通用的安全保障仍需在后续工作中结合正式中间件安全机制进一步推进。
...
- `defenseEffective(...)`
建议说明的问题：
- 直观展示“同一组 query 在防护前后为何出现 yes/no 对照”；
- 作为第七节规则结果与实验结果汇总的核心表格。
建议排版：
- 五列表格；
- “安全含义”一列尽量用完整短句，不只写关键词。
</SAURE_TODO>
...
建议说明的问题：
- 帮助读者快速把握“防护前后差异”；
- 支撑第 7.4 节的综合分析与收益/代价讨论。
建议排版：
- 五列表格；
- “风险变化”一列建议写成“显示层欺骗可达/不可达”“振荡存在/被阻断”等短语。
</SAURE_TODO>
...
综合第 5 节、第 6 节和本节前半部分可以看出，本文已经形成了较完整的“三层一致性”证据链。首先，在实验层面，`JointState` 伪造确实能够造成显示层状态欺骗，竞争性 publisher 确实能够造成姿态振荡；其次，在规则层面，攻击版 XSB 模型能够把上述现象分别归纳为 `misleadPoseDisplay(...)` 与 `oscillatingPoseDisplay(...)`，并进一步指出其前提依赖于 topic 可枚举、消息可学习、状态可伪造且缺乏来源控制；最后，在代码层面，防护节点通过重构消息流并加入输入校验，使得这两条攻击链在运行时被部分切断。

这种一致性非常重要，因为它避免了“实验只是偶然现象”或“规则只是抽象想象”的问题。相反，本研究中的现象、规则与实现之间形成了相互支撑关系：实验说明风险真实存在，规则说明风险为何成立，代码说明风险可以在何处被拦截。因此，本文的结论并非建立在单一视角上，而是建立在“现象可观测、逻辑可解释、实现可验证”的三重基础之上。

#### 7.4.2 防护收益与代价分析

从收益角度看，当前防护方案至少带来了三方面改进。第一，它切断了原始裸 `/joint_states` 对下游姿态显示链的直接影响路径，使攻击者不能再沿旧路径直接命中 `robot_state_publisher`；第二，它通过时间戳、关节名称和关节范围检查，提高了伪造状态消息被接受的门槛；第三，它通过“必须恰好存在一个 publisher”的约束，针对本文最有代表性的竞争性 publisher 攻击给出了有效的运行时拦截手段。

从代价角度看，这套方案本质上是一种偏完整性优先的 fail-closed 设计。也就是说，当系统无法确认输入唯一可信时，它选择拒绝转发，而不是继续使用不确定输入。这样做的好处是可以避免姿态显示继续被污染，但代价是局部可用性会受到影响，例如在检测到双 publisher 冲突时，姿态显示可能停止更新或短时冻结。对于研究型原型而言，这种权衡是可以接受的；但在实际工业系统中，还需要结合控制连续性需求、告警设计和运维流程进一步优化。

#### 7.4.3 研究边界与不足

尽管本文已经完成了从攻击复现、规则建模到防护验证的核心闭环，但仍存在若干边界与不足。首先，本次结项工作的主验证环境仍然是 ROS2 Foxy 下的虚拟显示链，实机平台虽已具备硬件基础，但未完成完整的物理执行层闭环验证，因此“物理层风险”更多体现为基于代码结构与桥接逻辑的潜在传播分析，而非大规模实机动作实验。其次，结项阶段曾尝试搭建 Ubuntu 22.04 与 ROS2 Humble 环境以推进 MoveIt2 与更高层轨迹控制链验证，但由于环境适配与时间约束，该增强路线未能纳入主实验闭环。

此外，本文的代码级防护仍属于应用层原型，其核心能力在于消息过滤与输入仲裁，而非严格意义上的密码学身份认证。它能够显著缓解已复现出的攻击场景，但尚不能替代 SROS2、DDS Security、证书体系或更完善的访问控制策略。最后，本文聚焦于 `JointState` 状态链这一条具有代表性的攻击与防护主线，并未覆盖 ROS2 机器人系统中全部可能的 service、action、参数接口或更复杂的多机器人协同场景。因此，本文更适合作为一个可复现、可解释、可扩展的研究样例，而不是对机器人安全问题的穷尽性覆盖。
...
- 研究边界。
建议说明的问题：
- 将本节分析压缩成便于答辩展示和报告查阅的总表；
- 有助于第八节结论与展望自然收束。
建议排版：
- 四列表格；
- “支撑证据”一列可简写为“图 7-1 / 表 7-1 / 日志”等。
</SAURE_TODO>
...
本文围绕 myCobot-280-arduino 在 ROS2 环境下的 `JointState` 状态链展开研究，完成了从攻击现象复现、规则推理建模到代码级防护验证的结项闭环。研究结果表明，在未加保护的 ROS2 显示链中，攻击者能够通过构造语义合法且时间戳有效的 `JointState` 消息影响 `robot_state_publisher` 与 RViz 的姿态显示结果，形成显示层状态欺骗；当合法 publisher 与恶意 publisher 在同一关键状态 topic 上并存时，系统会因缺乏来源鉴别和输入仲裁而出现竞争性状态振荡现象。
...
本文在研究与写作过程中使用了生成式人工智能工具作为辅助。AI 的主要参与方式包括：协助整理开题与中期阶段材料、协助梳理报告章节结构、根据实验记录生成初稿性文字说明、协助将 ROS2 攻击与防护实验现象转写为更规范的学术表达、辅助构建和整理 XSB 规则文件，以及协助完成部分代码级防护原型的方案设计、实现草稿与工程组织工作。此外，AI 还参与了实验记录的归纳、图表占位建议的生成以及 GitHub 仓库结构整理等工作。

需要明确的是，本文中的核心研究判断、实验方向选择、ROS2 虚拟环境搭建与操作、XSB 查询运行、攻击与防护现象观察、关键结果验证以及最终内容取舍均由作者本人完成。AI 生成的内容均经过人工审阅、修改和筛选后才被纳入最终报告；其中涉及实验结论的部分，均以作者实际运行得到的日志、截图、视频和查询结果为依据，而非直接采用未验证的自动生成结论。
