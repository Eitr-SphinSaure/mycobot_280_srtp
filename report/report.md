...
图 2-2 ROS2 显示链结构图占位。
建议内容：
- 用框图或消息流图表示 `joint_state_publisher -> /joint_states -> robot_state_publisher -> /tf -> RViz`；
- 可以在图中标出各节点角色与消息方向。
建议说明的问题：
- 让读者在进入攻击分析前先看懂系统正常运行链路；
- 明确 `/joint_states` 在整条链中的关键地位。
建议排版：
- 单图居中，适合用流程图或架构图；
...
建议说明的问题：
- 让读者提前建立第四节的阅读框架；
- 表明本文不是只做现象展示，而是做了形式化推理闭环。
建议排版：
- 单图流程图；
- 左到右或上到下结构均可，但建议与后文第四节保持同一术语。
</SAURE_TODO>
...
- 正常显示链中的节点与 topic；
- 攻击者所在位置；
- `/joint_states` 所处的关键边界；
- 如果版面允许，可在同图中加入防护后链路的对比。
建议说明的问题：
- 说明攻击者并不是直接篡改 RViz，而是通过状态链影响显示层；
- 体现“信任边界放得过宽”是风险根源。
建议排版：
- 推荐上下对照图：
  - 上：未防护；
...
<SAURE_TODO>
图 5-2 竞争性 publisher 实验图组占位。
建议内容：
1. 正常 GUI 控制下的姿态；
2. 攻击者注入下的姿态；
3. 双 publisher 并存时的振荡现象截图或示意图。
建议说明的问题：
- 直观呈现状态竞争；
- 与第五节前一实验形成“单源伪造 vs 多源竞争”的对照。
建议排版：
- 三联图；
- 如果无法准确截振荡，可用两张典型状态图加说明箭头。
</SAURE_TODO>
...
<SAURE_TODO>
图 7-2 竞争性 publisher 振荡实验图组占位。
建议内容：
1. 保持 GUI publisher 正常运行时的机械臂稳定姿态截图；
2. 恶意 publisher 加入后机械臂发生姿态切换或振荡的截图；
3. 如可从视频中截帧，建议选取 safe pose 与 attacker-defined pose 各一帧并并列展示；
4. 可附竞争性注入脚本终端输出截图作为辅助。
建议说明的问题：
- 说明“正常 publisher 仍在”并不意味着系统安全；
- 说明问题的根源是多源竞争而非简单的单次错误输入。
建议排版：
- 推荐左右对照或 2×2 图组；
- safe pose 与 attacker-defined pose 建议并列放置，视觉对比更明显。
</SAURE_TODO>
